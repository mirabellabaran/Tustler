...
namespace Tustler.UserControls
{
    ...
    public partial class TasksManager : UserControl
    {
        ...
        public TasksManager(AmazonWebServiceInterface awsInterface)
        {
            ...
            agent = new Agent(awsInterface, notificationsList);
            agent.NewUIResponse += Agent_NewUIResponse;
            agent.SaveArguments += Agent_SaveArguments;
            agent.TaskNameChanged += Agent_TaskNameChanged;
            agent.Error += Agent_Error;
        }

        private void UserControl_Loaded(object sender, RoutedEventArgs e)
        {
            ShowGlobalMessage("Parameter set", $"Task name set to {TaskName}");
        ...
        //private async Task ProcessResponse(TaskResponse response)
        //{
        ...
        private void RunTask()
        {
            ...
            //var queue = new ConcurrentQueue<TaskResponse>();

            agent.RunTask(responseStream);     //.ConfigureAwait(true);
        }

        private async void Agent_NewUIResponse(object? sender, TaskResponse response)
        {
            //await ProcessResponse(response).ConfigureAwait(true);
            await Dispatcher.InvokeAsync(() =>
            {
                taskResponses.Add(response);
                lbTaskResponses.ScrollIntoView(response);
        ...
        private async void Agent_TaskNameChanged(object? sender, string taskName)
        {
            await Dispatcher.InvokeAsync(() =>
            {
                this.TaskName = taskName;   // will throw if task name is unknown
                RunTask();
            });
        }
            ...
            void RunUIResponseSelect(UITaskArguments parameterInfo)
            {
                ...
                switch (parameterInfo.TaskArguments.First())
                {
                    case UITaskArgument.Bucket bucketArg:
                        ...
                        agent.AddArgument(TaskResponse.NewSetBucket(bucket));
                        break;
                    case UITaskArgument.FilePath filePathArg:
                        var filePath = filePathArg.Item;
                        agent.AddArgument(TaskResponse.NewSetFilePath(filePath));
                        break;
                    case UITaskArgument.FileMediaReference mediaReferenceArg:
                        var mediaReference = mediaReferenceArg.Item;
                        agent.AddArgument(TaskResponse.NewSetFileMediaReference(mediaReference));
                        break;
                    case UITaskArgument.TranscriptionLanguageCode transcriptionLanguageCodeArg:
                        ...
                        agent.AddArgument(TaskResponse.NewSetTranscriptionLanguageCode(transcriptionLanguageCode));
                        break;
                    case UITaskArgument.TranslationLanguageCode translationLanguageCodeArg:
                        ...
                        agent.AddArgument(TaskResponse.NewSetTranslationLanguageCode(translationLanguageCode));
                        break;
                    case UITaskArgument.VocabularyName vocabularyNameArg:
                        var vocabularyName = vocabularyNameArg.Item;
                        agent.AddArgument(TaskResponse.NewSetVocabularyName(vocabularyName));
                        break;
                    default:
