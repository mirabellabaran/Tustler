...
namespace Tustler.UserControls
{
    ...
    public partial class TasksManager : UserControl, IOwnerType
    {
        ...
        private bool defaultArgumentsAdded = false;

        public static readonly DependencyProperty TaskSpecifierProperty = DependencyProperty.Register("TaskSpecifier", typeof(TaskFunctionSpecifier), typeof(TasksManager));
        ...
        private bool PrepareTaskFirstRun(string taskFolderPath)
        {
            ...
            // set the root task
            agent.PushRootTask(TustlerServicesLib.ApplicationSettings.FileCachePath, this.TaskSpecifier);
        ...
        private async void StartTask_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            switch (runMode)
            {
                ...
                case RunMode.Stopped:
                    ...
                    {
                        //await FirstRun().ConfigureAwait(false);   // no good, as it adds another instance of the root task on the execution stack

                        // create a new Agent and then restart
                        var taskFolderPath = Path.Combine(TustlerServicesLib.ApplicationSettings.FileCachePath, this.TaskSpecifier.TaskName);
                        await StartNewAsync(taskFolderPath).ConfigureAwait(false);
                    }
        ...
        //{
        //    if (agent.NextTask() is TaskFunctionSpecifier nextTaskSpecifier)
        //    {
        //    //}
        ...
        private async void UIResponse_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            async Task RunUIResponseSelectDefaultArgumentsAsync(UITaskArguments parameterInfo)
            {
                ...
                if (taskEvents.Length > 0)
                {
                    ...
                    this.defaultArgumentsAdded = true;
                }
            ...
            async Task RunUIResponseRestartTaskAsync(UITaskArguments parameterInfo)
            {
                ...
                if (hasCompleted)
                {
                    ...
                    agent.PushTask(this.TaskSpecifier);
                }
