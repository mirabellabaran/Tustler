...
type StandardArgument =
    ...
    | SetSubTaskInputs of IEnumerable<IRequestIntraModule>  // allow root tasks to preevaluate the input requests for their constituent sub-tasks
    ...
    override this.ToString() =
        ...
        | SetSubTaskInputs requests -> (sprintf "SetSubTaskInputs: %d requests" (Seq.length requests))
...
and StandardShareIntraModule(arg: StandardArgument) =
    interface IShareIntraModule with
        ...
        member this.Description () =
            ...
            | SetSubTaskInputs requests -> (sprintf "SubTaskInputs: %s" (System.String.Join(", ", Seq.map (fun (request: IRequestIntraModule) -> request.ToString()) requests)))
        member this.AsBytes serializerOptions =    // returns either a UTF8-encoded string or a UTF8-encoded Json document as a byte array
            ...
            | SetSubTaskInputs requests -> JsonSerializer.SerializeToUtf8Bytes(requests)
        member this.Serialize writer serializerOptions =
            ...
            | SetSubTaskInputs requests -> writer.WritePropertyName("SetSubTaskInputs"); JsonSerializer.Serialize<IEnumerable<IRequestIntraModule>>(writer, requests)
    ...
    static member Deserialize propertyName (jsonString:string) serializerOptions (flagResolverDictionary: Dictionary<string, Func<string, Dictionary<string, ISaveFlagSet>, Dictionary<string, ISaveFlagSet>>>) =
        let standardArgument =
            ...
            | "SetSubTaskInputs" ->
                let data = JsonSerializer.Deserialize<IEnumerable<IRequestIntraModule>>(jsonString, serializerOptions)
