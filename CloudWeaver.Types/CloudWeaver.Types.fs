...
// an attribute for decorating a Task Function; tells the Agent that the task function is a root task (a task that defines a sequence of sub-tasks)
type RootTask() = inherit System.Attribute()

// an attribute for decorating a Task Function; tells the Agent to enable logging of TaskEvents generated by the attribute-decorated task function
...
type TaskFunctionQueryMode =
    ...
    | SubTasks                  // show the sequence of sub-tasks that constitute the task function (an empty sequence for all but root tasks)
...
/// The combined set of inputs for all sub-tasks of a root task (excluding internally resolved arguments and arguments generated by sub-tasks earlier in the sequence)
type SubTaskInputs(requests: IEnumerable<IRequestIntraModule>) =
    let mutable _requests = requests
    
    member this.Requests with get() = _requests and set(value) = _requests <- value

    new() = SubTaskInputs(null)
    override this.ToString() = sprintf "SubTaskInputs: %s" (System.String.Join(", ", Seq.map (fun (request: IRequestIntraModule) -> request.ToString()) this.Requests))
...
type StandardRequest =
    ...
    | RequestSubTaskInputs          // get all the Requests for each sub-task in a root task
    with
    override this.ToString() =
        match this with
        ...
        | RequestSubTaskInputs -> "RequestSubTaskInputs"
    static member fromString(label) =
        let arg = BaseUtilities.fromString<StandardRequest>(label)
        if arg.IsSome then
            arg.Value
        else
...
and StandardShareIntraModule(arg: StandardArgument) =
    interface IShareIntraModule with
        ...
        member this.Description () =
            ...
            | SetSubTaskInputs requests -> (sprintf "SubTaskInputs: %s" (System.String.Join(", ", Seq.map (fun (request: IRequestIntraModule) -> request.ToString()) requests.Requests)))
        member this.AsBytes serializerOptions =    // returns either a UTF8-encoded string or a UTF8-encoded Json document as a byte array
            ...
            | SetSubTaskInputs requests -> JsonSerializer.SerializeToUtf8Bytes(requests.Requests)
        member this.Serialize writer serializerOptions =
            ...
            | SetFilePath filePath -> writer.WritePropertyName("SetFilePath"); JsonSerializer.Serialize<FilePickerPath>(writer, filePath, serializerOptions)
            | SetSubTaskInputs requests -> writer.WritePropertyName("SetSubTaskInputs"); JsonSerializer.Serialize<SubTaskInputs>(writer, requests, serializerOptions)
    ...
    static member Deserialize propertyName (jsonString:string) serializerOptions (flagResolverDictionary: Dictionary<string, Func<string, Dictionary<string, ISaveFlagSet>, Dictionary<string, ISaveFlagSet>>>) =
        let standardArgument =
            ...
            | "SetSubTaskInputs" ->
                let data = JsonSerializer.Deserialize<SubTaskInputs>(jsonString, serializerOptions)
                StandardArgument.SetSubTaskInputs data
...
module public PatternMatchers =
    ...
    let private (| SubTaskInputs |) argMap =
        let key = StandardRequestIntraModule(StandardRequest.RequestSubTaskInputs)
        match (lookupArgument key argMap) with
        | Some(SetSubTaskInputs arg) -> Some(arg)
        | None -> None
        | _ -> invalidArg "arg" "Expecting a sequence of Requests"
    ...
    let getSubTaskInputs argMap = match (argMap) with | SubTaskInputs arg -> arg
