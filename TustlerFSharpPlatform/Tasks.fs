...
type TaskResponse =
    ...
    | FilePath of string
    | FileUploadSuccess of S3MediaReference option
...
type TranscribeAudioArgs with
    member x.AllSet =  x.S3BucketName.IsSome && x.FileMediaReference.IsSome && x.TranscriptionLanguageCode.IsSome && x.VocabularyName.IsSome
    ...
    member x.Update response =
        ...
        | TaskResponse.S3MediaReference media -> { x with S3MediaReference = Some(media) }
...
module public Tasks =            
    ...
    let TranscribeAudio (common_arguments: ITaskArgumentCollection) (resolvable_arguments: InfiniteList<MaybeResponse>) =
        ...
        let uploadMediaFile argsRecord awsInterface notifications =
            ...
            seq {
                yield! getNotificationResponse notifications
                yield TaskResponse.FileUploadSuccess (if success then Some(S3MediaReference(bucketName, newKey, media.MimeType, media.Extension)) else None)
            }
        ...
        seq {
            ...
            let defaultArgs = TaskArgumentRecord.TranscribeAudio {
            ...
            if transcribeAudioArgs.AllSet then
                ...
                yield! resolveLocally resolvable_arguments transcribeAudioArgs awsInterface notifications [|
                    uploadMediaFile; startTranscription
                |]
